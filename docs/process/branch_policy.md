...
- stale worktree branches: `worktree-agent-*` are not release sources and must stay untouched for
  `v0.1.0-rc.1`
- current release-prep branch: `rewrite/go-tui`
...
- `v0.1.0-rc.1` must point to the tested merge commit on `main`, not to a worktree branch or local
  release-prep commit
- `v0.1.0` stable promotion requires the full RC soak and promotion gates in
