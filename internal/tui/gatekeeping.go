...
func firmwareDisabledReason(device core.AppDevice, unsafeAcknowledged, writeLockUntilRestart bool) string {
	switch {
...
func mappingDisabledReason(device core.AppDevice, writeLockUntilRestart bool) string {
...
func blockedLinesForDevice(device core.AppDevice, unsafeAcknowledged, advancedMode, acknowledgedRisk, writeLockUntilRestart bool) []string {
	var lines []string
	if reason := firmwareDisabledReason(device, unsafeAcknowledged, writeLockUntilRestart); reason != "" {
		lines = append(lines, "Firmware update: "+reason)
	}
	if reason := mappingDisabledReason(device, writeLockUntilRestart); reason != "" {
